...
# 'visibility', 'read more', 'show', 'hide'
NAME_FILTER_KEYS = ['cancel', 'sso', 'deny', 'refresh', 'call', 'install', 'delete', 'disagree', 'logout', 'no, thanks',
                    'clear', "don't allow", 'not now']

ID_FILTER_KEYS = ['cancel', 'delete', 'clearpassword', 'clearaccount', 'ivclear']

THIRD_PARTY_KEYS = ['facebook', 'linkedin', 'google', 'twitter', 'github', 'wechat', 'qq', 'taobao']

COUNTRY_CODE_FILTER_KEYS = ['countrycode', 'country_code', 'country code', 'selectedcountry', 'select_country']
...
class Node:
    ...
    def is_register(self):
        ...
        if has_filter_key(split_resource_id(self.attribute['resourceID']), register_keys):
            return True
        if has_filter_key(self.attribute['name'], register_keys):
            return True
        if self.__children is None:
            return False
        for ce in self.__children:
            if not has_filter_key(ce.attribute['name'], NAME_FILTER_KEYS):
                if has_filter_key(ce.attribute['name'], register_keys) or has_filter_key(
                        split_resource_id(ce.attribute['resourceID']), register_keys):
                    return True
        return False

    def is_login(self):
        login_keys = ['log in', 'sign in', 'login', 'signin']
        if has_filter_key(self.attribute['name'], login_keys):
            return True
        if self.__children is None:
            return False
        for ce in self.__children:
            if not has_filter_key(ce.attribute['name'], NAME_FILTER_KEYS):
                if has_filter_key(ce.attribute['name'], login_keys):
                    return True
        return False

    def is_back(self):
        back_keywords = ['back', 'navigate up', 'close', 'cancel', 'quit']
        if self.attribute['resourceID'] and 'back' in split_resource_id(self.attribute['resourceID'].lower()):
            return True
        elif has_filter_key(self.attribute['name'], back_keywords):
            return True
        else:
            return False
    ...
    def is_third_party(self):
        ...
        id_text = split_resource_id(self.attribute['resourceID']).lower()
        text_arr = name_text.split(' ') + name_text.split('_') + id_text.split(' ') + id_text.split('_')
        if has_filter_key(text_arr, THIRD_PARTY_KEYS):
            return True
        elif has_filter_key(name_text, THIRD_PARTY_KEYS) or has_filter_key(id_text, THIRD_PARTY_KEYS):
            return True
        else:
            if self.__children is None:
                return False
            for c in self.__children:
                ...
                id_text = split_resource_id(c.attribute['resourceID']).lower()
                text_arr = name_text.split(' ') + name_text.split('_') + id_text.split(' ') + id_text.split('_')
                if has_filter_key(text_arr, THIRD_PARTY_KEYS):
                    return True
                elif has_filter_key(name_text, THIRD_PARTY_KEYS) or has_filter_key(id_text, THIRD_PARTY_KEYS):
    ...
    def is_expand(self):
        ...
        if has_filter_key(split_resource_id(self.attribute['resourceID']), expand_keys):
            return True
        if self.__children is not None:
            for c in self.__children:
                if has_filter_key(split_resource_id(c.attribute['resourceID']), expand_keys):
                    return True
            return False
        else:
            return False
    ...
    def is_country_code(self):
        if has_filter_key(split_resource_id(self.attribute['resourceID']), COUNTRY_CODE_FILTER_KEYS):
            return True
        else:
            if self.__children is None:
                return False
            for c in self.__children:
                if has_filter_key((split_resource_id(c.attribute['resourceID'])), COUNTRY_CODE_FILTER_KEYS) or \
                        c.attribute['text'] == '+86':
                    return True
            return False
    ...
    def need_filter(self):
        ...
        if self.get_parent() is not None and self.get_parent().attribute['classType'] == 'android.widget.GridView':
            if self.is_equal(self.__parent.get_children()[0]):
                ...
                return True
        if has_filter_key(self.attribute['name'], NAME_FILTER_KEYS) or has_filter_key(
                split_resource_id(self.attribute['resourceID']), ID_FILTER_KEYS):
            return True
        ccs = self.__children
        if ccs is not None:
            for c in ccs:
                if has_filter_key(c.attribute['name'], NAME_FILTER_KEYS):
                    return True
            return False
        return False
    ...
    def no_need_deep_explore(self):
        ...
        if has_filter_key(self.attribute['name'], name_no_deep_keys) or has_filter_key(
                split_resource_id(self.attribute['resourceID']), id_no_deep_keys):
            return True
        if self.__children is not None:
            for c in self.__children:
                if has_filter_key(c.attribute['name'], name_no_deep_keys) or has_filter_key(
                        split_resource_id(c.attribute['resourceID']), id_no_deep_keys):
                    return True
            return False
        else:
            return False
    ...
    def is_user_image(self):
        ...
        if has_filter_key(split_resource_id(self.attribute['resourceID']), image_keys):
            return True
        if self.__children is not None:
            for c in self.__children:
                if has_filter_key(split_resource_id(c.attribute['resourceID']), image_keys):
                    return True
            return False
        else:
            return False
    ...
    def is_date_of_sth(self):
        ...
        if has_filter_key(self.attribute['name'], date_keywords):
            return True
        if self.__children is not None:
            for c in self.__children:
                if has_filter_key(c.attribute['name'], date_keywords):
                    return True
            return False
        else:
            return False
    ...
    def is_privacy_policy(self):
        ...
        if has_filter_key(self.attribute['name'], privacy_keys):
            return True
        else:
            return False
    ...
    def is_search_edit_text(self):
        ...
        if self.attribute['classType'] == 'android.widget.EditText':
            if has_filter_key(split_resource_id(self.attribute['resourceID']), search_keys) or has_filter_key(
...
class EditNode(Node):
    ...
    def add_hint(self, strings):
        if isinstance(strings, list):
            self.hints = list(set(self.hints).union(strings))
...
def split_resource_id(idstring):
...
def is_same_bound(arraybound, bound):
    if type(bound) == dict:
        if arraybound[0][0] == bound["left"] and arraybound[0][1] == bound["top"] and arraybound[1][0] == bound[
            "right"] and \
                arraybound[1][1] == bound["bottom"]:
            return True
        else:
            return False
    elif type(bound) == list:
        if arraybound[0][0] == bound[0][0] and arraybound[0][1] == bound[0][1] and arraybound[1][0] == bound[1][0] and \
                arraybound[1][1] == bound[1][1]:
            return True
        else:
            return False
    else:
        print("Cannot handle bound type:", type(bound), bound)
        return False
