...
class XmlTree:
    def __init__(self, file="", xml_string=None):
        ...
        if self.DrawerLayoutNode:
            if len(self.DrawerLayoutNode) == 1:
                ...
                # more than one DrawerLayoutNode
                pass
    ...
    def parse_nodes(self):
        ...
        if globalConfig.TETranslateON:
            import langid
            from googletrans import Translator
            import traceback
            te_translator = Translator(service_urls=[globalConfig.TETranslaterServiceUrl])
            for nd in self.AllNodes:
                try:
                    if nd.attribute["text"] and langid.classify(nd.attribute["text"])[0] != "en":
                        nd.attribute["text"] = te_translator.translate(clean_encode(nd.attribute["text"])).text
                    if nd.attribute["contentDesc"] and langid.classify(nd.attribute["contentDesc"])[0] != "en":
                        nd.attribute["contentDesc"] = te_translator.translate(
                            clean_encode(nd.attribute["contentDesc"])).text
                    if nd.attribute['contentDesc']:
                        nd.attribute['name'] = nd.attribute['contentDesc']
                    else:
                        nd.attribute['name'] = nd.attribute['text']
                except:
                    traceback.print_exc()
                    continue
        tmpidx = 0
        for n in self.AllNodes:
            if n.attribute["classType"] in EDIT_CLASS:
        ...
        for node in self.AllNodes:
            ...
            if className == 'android.support.v4.widget.DrawerLayout':
                ...
                # Remove TextInputlayout which don't has child node
                ...
                # remove influence cased by cursor
                ...
                'clickable'] == 'true' and split_resource_id(node.attribute['resourceID']) == 'container':
                if node not in self.IdentifyEditNodes:
                    self.IdentifyEditNodes.append(node)
            ...
            if node.attribute['clickable'] == 'true' and node.attribute['enable'] == 'false':
                ...
                if node.is_back():  # ignore back button
                ...
                elif className not in ADDITIONAL_CLASS and has_filter_key(
                        split_resource_id(node.attribute['resourceID']),
    ...
    def no_need_input(self):
        ...
        if len(self.EditNodes) == 1:
            if 'picker' in split_resource_id(self.EditNodes[0].attribute['resourceID']) and self.EditNodes[0].attribute[
                'text']:
                return True
